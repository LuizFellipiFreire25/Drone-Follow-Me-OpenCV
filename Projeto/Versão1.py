...
while True:
    ...
    if modo_atual == MODO_RASTREAMENTO:
        ...
        if ok:
            # Lógica de controle (a mesma do projeto anterior)
            p1 = (int(bbox[0]), int(bbox[1]))
            p2 = (int(bbox[0] + bbox[2]), int(bbox[1] + bbox[3]))
            cv2.rectangle(frame, p1, p2, cor_sucesso, 2, 1)

            area_atual = bbox[2] * bbox[3]
            if area_atual > area_referencia * 1.15:
                comando_distancia = "AFASTAR"
            elif area_atual < area_referencia * 0.85:
                comando_distancia = "APROXIMAR"
            else:
            ...
            zona_morta = 50
            if centro_obj_x < centro_frame_x - zona_morta:
                comando_posicao = "MOVER ESQUERDA"
            elif centro_obj_x > centro_frame_x + zona_morta:
                comando_posicao = "MOVER DIREITA"
            else:
                comando_posicao = "CENTRALIZADO"

            comando_final = f"{comando_posicao} | {comando_distancia}"
            cv2.putText(frame, comando_final, (10, 30),
                        fonte, 0.75, cor_info, 2)
        else:
            # Rastreamento falhou!
            cv2.putText(frame, "Alvo Perdido!", (100, 80),
                        fonte, 0.75, cor_falha, 2)
            modo_atual = MODO_DETECCAO  # Volta para o modo de detecção
